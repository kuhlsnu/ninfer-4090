...
#pragma unroll
    ...
    // K/SplitK and the epilogue adds the slices in fp32, so the split routes are structurally
    // more accurate than the unsplit one. Give unsplit the same shape without a grid partition:
    // sum the two halves of K into separate registers and add them at the end. That is the
    // arithmetic Split2 performs, so it costs one extra accumulator set (16 registers here) and
    // one predicated register swap, with no extra CTA, workspace traffic or grid synchronization.
    //
    // Halving the chain also halves the running sum the later terms round against, so the error
    // falls by more than the square root of the length ratio. Breaking the single dependency
    // chain in two additionally exposes instruction-level parallelism the MMA pipeline was
    // stalling on, so the route also gets faster. a_head and b_head fold away on the cooperative
    // routes.
